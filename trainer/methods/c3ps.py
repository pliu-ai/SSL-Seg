"""C3PS (Conditional Cross-pseudo-label with Patch-level Supervision) training loop."""
import os
import numpy as np
import torch
    ...
    for epoch_num in iterator:
        for i_batch in range(fixed_iters):
            ...
            with autocast():
                ...
                if trainer.current_iter < trainer.began_semi_iter or overlap_filter1.sum() == 0:
        ...
        if trainer.use_PL:
            ...
            for i_batch, sampled_batch in enumerate(trainer.dataloader_pl):
                ...
                loss2 = 0.1 * (
                    ...
                    trainer.dice_loss_con(outputs_soft2, label_batch.unsqueeze(1))
                )
                model2_loss = loss2
                ...
                trainer.tensorboard_writer.add_scalar(
                    'loss/model2_loss_pl', model2_loss, trainer.current_iter
                )
                trainer.logging.info(
