...
def _prepare_condition(condition, batch_size, num_conditions, device):
    """Convert condition input to multi-hot vector of shape (B, num_conditions).

    Accepts:
        - int or scalar tensor         → single class index
        - (B,) or (B,1) long tensor    → per-sample class index
        - (B, num_conditions) float tensor → already multi-hot, pass through
    """
    ...
    # Already multi-hot: (B, num_conditions) or (1, num_conditions)
    if condition.ndim == 2 and condition.shape[-1] == num_conditions:
    ...
    return _condition_to_vector(condition, num_conditions)
...
class ScalarCondition(nn.Module):
    ...
    def forward(self, cond_vec, spatial_shape):
        """cond_vec: (B, num_conditions) multi-hot. Extract argmax as scalar."""
        B = cond_vec.shape[0]
        cond = cond_vec.argmax(dim=1).float().view(B, 1, 1, 1, 1)
...
class ConditionEmbedding(nn.Module):
    ...
    def forward(self, cond_vec, spatial_shape):
        """
        Args:
            cond_vec: (B, num_conditions) multi-hot float tensor.
...
class unet_3D_Condition(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True,
        ...
        self.num_conditions = num_conditions if num_conditions is not None else 16
        ...
        if condition_mode == 'scalar':
            ...
            self.embed_dim = embed_dim
            self.cond_embed = ConditionEmbedding(self.num_conditions, embed_dim)
            ed = embed_dim
        elif condition_mode == 'film':
            ed = 0
            self.cond_enc = ConditionEncoder(self.num_conditions, cond_dim)
    ...
    def _inject(self, feature, cond_vec, cond_emb, film_layer):
        if self.condition_mode in ('scalar', 'concat'):
            c = self.cond_embed(cond_vec, feature.shape[2:])
            return torch.cat((c, feature), dim=1)
        else:
            return film_layer(feature, cond_emb)

    def forward(self, inputs, condition=1):
        cond_vec = _prepare_condition(condition, inputs.shape[0],
                                      self.num_conditions, inputs.device)
        cond_emb = self.cond_enc(cond_vec) if self.condition_mode == 'film' else None

        conv1 = self.conv1(inputs)
        maxpool1 = self.maxpool1(conv1)
        maxpool1 = self._inject(maxpool1, cond_vec, cond_emb,
                                getattr(self, 'film_mp1', None))

        conv2 = self.conv2(maxpool1)
        maxpool2 = self.maxpool2(conv2)
        maxpool2 = self._inject(maxpool2, cond_vec, cond_emb,
                                getattr(self, 'film_mp2', None))

        conv3 = self.conv3(maxpool2)
        maxpool3 = self.maxpool3(conv3)
        maxpool3 = self._inject(maxpool3, cond_vec, cond_emb,
                                getattr(self, 'film_mp3', None))

        conv4 = self.conv4(maxpool3)
        maxpool4 = self.maxpool4(conv4)
        maxpool4 = self._inject(maxpool4, cond_vec, cond_emb,
                                getattr(self, 'film_mp4', None))

        center = self.center(maxpool4)
        center = self.dropout1(center)
        center = self._inject(center, cond_vec, cond_emb,
                              getattr(self, 'film_center', None))

        up4 = self.up_concat4(conv4, center)
        up4 = self._inject(up4, cond_vec, cond_emb,
                           getattr(self, 'film_up4', None))

        up3 = self.up_concat3(conv3, up4)
        up3 = self._inject(up3, cond_vec, cond_emb,
                           getattr(self, 'film_up3', None))

        up2 = self.up_concat2(conv2, up3)
        up2 = self._inject(up2, cond_vec, cond_emb,
                           getattr(self, 'film_up2', None))

        up1 = self.up_concat1(conv1, up2)
        up1 = self.dropout2(up1)
        up1 = self._inject(up1, cond_vec, cond_emb,
...
class Unet3DConditionBottom(nn.Module):
    ...
    def forward(self, inputs, condition=1):
        ...
        if self.condition_mode in ('scalar', 'concat'):
            c = self.cond_embed(cond_vec, center.shape[2:])
            center = torch.cat((c, center), dim=1)
        else:
            cond_emb = self.cond_enc(cond_vec)
...
class SharedEncoderCondUNet3D(nn.Module):
    """Shared-encoder UNet with two decoders: multi-class (dec1) + condition head (dec2).

    During training (self.training=True):
        forward(x, condition) -> (out1, out2)
    During eval:
        forward(x)            -> out1   (multi-class, for model1 eval)
        forward(x, condition) -> out2   (binary, for condition eval)
    """

    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True,
                 in_channels=1, is_batchnorm=True,
                 num_conditions=None, embed_dim=8,
                 condition_mode='film', cond_dim=32, cond_out_classes=2):
        super(SharedEncoderCondUNet3D, self).__init__()
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
        self.condition_mode = condition_mode
        self.num_conditions = num_conditions if num_conditions is not None else 16
        self.cond_out_classes = int(cond_out_classes)
        ...
        self.center = UnetConv3(filters[3], filters[4], self.is_batchnorm,
                                kernel_size=(3, 3, 3), padding_size=(1, 1, 1))
        self.enc_dropout = nn.Dropout(p=0.3)

        # ---- Decoder 1: standard multi-class ----
        self.dec1_up4 = UnetUp3_CT(filters[4], filters[3], is_batchnorm)
        self.dec1_up3 = UnetUp3_CT(filters[3], filters[2], is_batchnorm)
        self.dec1_up2 = UnetUp3_CT(filters[2], filters[1], is_batchnorm)
        self.dec1_up1 = UnetUp3_CT(filters[1], filters[0], is_batchnorm)
        self.dec1_dropout = nn.Dropout(p=0.3)
        self.dec1_final = nn.Conv3d(filters[0], n_classes, 1)

        # ---- Decoder 2: condition binary ----
        if condition_mode == 'film':
            ed = 0
            self.cond_enc = ConditionEncoder(self.num_conditions, cond_dim)
            self.film_center = FiLMLayer(cond_dim, filters[4])
            self.film_up4 = FiLMLayer(cond_dim, filters[3])
            ...
            raise ValueError(
                f"SharedEncoder only supports 'film' or 'concat', got: {condition_mode}"
            )

        self.dec2_up4 = UnetUp3_CT(filters[4] + ed, filters[3], is_batchnorm)
        self.dec2_up3 = UnetUp3_CT(filters[3] + ed, filters[2], is_batchnorm)
        self.dec2_up2 = UnetUp3_CT(filters[2] + ed, filters[1], is_batchnorm)
        self.dec2_up1 = UnetUp3_CT(filters[1] + ed, filters[0], is_batchnorm)
        self.dec2_dropout = nn.Dropout(p=0.3)
        self.dec2_final = nn.Conv3d(filters[0] + ed, self.cond_out_classes, 1)
    ...
    def encode(self, inputs):
        ...
        maxpool4 = self.maxpool4(conv4)
        center = self.center(maxpool4)
        center = self.enc_dropout(center)
        return conv1, conv2, conv3, conv4, center

    def decode1(self, conv1, conv2, conv3, conv4, center):
        up4 = self.dec1_up4(conv4, center)
        up3 = self.dec1_up3(conv3, up4)
        up2 = self.dec1_up2(conv2, up3)
        up1 = self.dec1_up1(conv1, up2)
        up1 = self.dec1_dropout(up1)
        return self.dec1_final(up1)

    def _inject2(self, feature, cond_vec, cond_emb, film_layer):
    ...
    def decode2(self, conv1, conv2, conv3, conv4, center, cond_vec, cond_emb):
        center = self._inject2(center, cond_vec, cond_emb,
                               getattr(self, 'film_center', None))
        up4 = self.dec2_up4(conv4, center)
        up4 = self._inject2(up4, cond_vec, cond_emb,
                            getattr(self, 'film_up4', None))
        up3 = self.dec2_up3(conv3, up4)
        up3 = self._inject2(up3, cond_vec, cond_emb,
                            getattr(self, 'film_up3', None))
        up2 = self.dec2_up2(conv2, up3)
        up2 = self._inject2(up2, cond_vec, cond_emb,
                            getattr(self, 'film_up2', None))
        up1 = self.dec2_up1(conv1, up2)
        up1 = self.dec2_dropout(up1)
        up1 = self._inject2(up1, cond_vec, cond_emb,
                            getattr(self, 'film_up1', None))
        return self.dec2_final(up1)

    def forward(self, inputs, condition=None):
        conv1, conv2, conv3, conv4, center = self.encode(inputs)

        if condition is None:
            return self.decode1(conv1, conv2, conv3, conv4, center)

        cond_vec = _prepare_condition(condition, inputs.shape[0],
                                      self.num_conditions, inputs.device)
        cond_emb = self.cond_enc(cond_vec) if self.condition_mode == 'film' else None
        out2 = self.decode2(conv1, conv2, conv3, conv4, center, cond_vec, cond_emb)

        if self.training:
            out1 = self.decode1(conv1, conv2, conv3, conv4, center)
            return out1, out2
        return out2

    def encoder_parameters(self):
        enc_modules = [self.conv1, self.conv2, self.conv3, self.conv4,
                       self.center, self.enc_dropout]
        for m in enc_modules:
            yield from m.parameters()

    def decoder1_parameters(self):
        dec1_modules = [self.dec1_up4, self.dec1_up3, self.dec1_up2,
                        self.dec1_up1, self.dec1_dropout, self.dec1_final]
        for m in dec1_modules:
            yield from m.parameters()

    def decoder2_parameters(self):
        dec2_modules = [self.dec2_up4, self.dec2_up3, self.dec2_up2,
                        self.dec2_up1, self.dec2_dropout, self.dec2_final]
        if self.condition_mode == 'film':
            dec2_modules.extend([self.cond_enc, self.film_center, self.film_up4,
                                 self.film_up3, self.film_up2, self.film_up1])
        elif self.condition_mode == 'concat':
            dec2_modules.append(self.cond_embed)
        for m in dec2_modules:
            yield from m.parameters()
